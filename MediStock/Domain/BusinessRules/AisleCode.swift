...
enum AisleCode {
    ...
    /// Pass `nil` to get just the code.
    /// This rule doesn't know about localization — callers resolve and pass in whatever label they want.
    static func format(code: String, aisleLabel: String?) -> String {
        guard let aisleLabel else { return code }
        return "\(aisleLabel) \(code)"
    }

    /// Removes a redundant label word (e.g. "Rayon"/"Aisle") a user may have typed along with the code itself.
    /// So the stored code stays just the code (e.g. "Rayon AD56" -> "AD56"). Case-insensitive.
    /// Assumes the label itself is never part of a real code.
    /// An accepted edge case given how rare and impractical that would be in practice.
    ...
    /// Orders two aisle codes the way Finder orders file names.
    /// Digit runs compare numerically ("AD2" before "AD10"), everything else compares as text.
